using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Linq;
using System.Net;
using System.Web;
using System.Web.Mvc;
using ProjetoInicio.Models;

namespace ProjetoInicio.Controllers
{
    public class TipoDeContratosController : Controller
    {
        private Contexto db = new Contexto();

        // GET: TipoDeContratos
        public ActionResult Index()
        {
            return View(db.TiposDeContratos.ToList());
        }

        // GET: TipoDeContratos/Details/5
        public ActionResult Details(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            TipoDeContrato tipoDeContrato = db.TiposDeContratos.Find(id);
            if (tipoDeContrato == null)
            {
                return HttpNotFound();
            }
            return View(tipoDeContrato);
        }

        // GET: TipoDeContratos/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: TipoDeContratos/Create
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "Id,Descricao,MesesDeAtraso,PrimeiroAluguel")] TipoDeContrato tipoDeContrato)
        {
            if (ModelState.IsValid)
            {
                db.TiposDeContratos.Add(tipoDeContrato);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(tipoDeContrato);
        }

        // GET: TipoDeContratos/Edit/5
        public ActionResult Edit(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            TipoDeContrato tipoDeContrato = db.TiposDeContratos.Find(id);
            if (tipoDeContrato == null)
            {
                return HttpNotFound();
            }
            return View(tipoDeContrato);
        }

        // POST: TipoDeContratos/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,Descricao,MesesDeAtraso,PrimeiroAluguel")] TipoDeContrato tipoDeContrato)
        {
            if (ModelState.IsValid)
            {
                db.Entry(tipoDeContrato).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(tipoDeContrato);
        }

        // GET: TipoDeContratos/Delete/5
        public ActionResult Delete(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            TipoDeContrato tipoDeContrato = db.TiposDeContratos.Find(id);
            if (tipoDeContrato == null)
            {
                return HttpNotFound();
            }
            return View(tipoDeContrato);
        }

        // POST: TipoDeContratos/Delete/5
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id)
        {
            TipoDeContrato tipoDeContrato = db.TiposDeContratos.Find(id);
            db.TiposDeContratos.Remove(tipoDeContrato);
            db.SaveChanges();
            return RedirectToAction("Index");
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }
    }
}
